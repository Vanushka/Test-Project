...
class Dropdown extends Component { // Создали класс Dropdown, унаследовали его от Component
  constructor(props) { // Добавили constructor и первым аргументом у него будет props.
    //Конструктор вызывается у подкласса для доступа к this.
    ...
    // создали новую переменную isOpened в state и поставили её значение false
  }
  ...
  toggleState() { // здесь мы описываем метод toggleState
    ...
    // Очень Важно, откуда вызывается setState - его нельзя вызвать в render, и нельзя вызвать в  constructor
    // setState создает очередь изменений состояния, при воздействии на this
  }
  ...
  render() { // вызываем метод render
    ...
    // делаем мы это, для проверки работы isOpened
    let dropdownText; // создаём переменную dropdownText
    if (this.state.isOpened) { // Написали if условие которое будет выводить текст в div при нажати, когда state будет isOpened: true
